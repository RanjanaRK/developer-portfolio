  ...
  const linkName = [
    {
      ...
      link: "mailto:ranjanakumari.me@gmail.com",
    },
    {
      id: 2,
      name: "WhatsApp",
        ...
        <div className="flex flex-col items-center justify-center gap-4">
          ...
          <div className="flex items-center justify-center gap-3">
            {linkName.map((i) => (
              <div
                key={i.id}
                className="flex flex-row items-center justify-center gap-x-4 md:gap-x-6"
              >
                <Link
                  className="text-foreground hover:text-muted-foreground lowercase underline-offset-4 transition-colors duration-300 ease-in-out hover:underline"
                  href={i.link}
                  target="_blank"
                >
                  <span className="hidden sm:block">{i.name}</span>
                  <span className="sm:hidden">{i.Icon}</span>
                </Link>
                <div className="bg-woodsmoke-800/5 dark:bg-woodsmoke-800/80 h-6 w-px border"></div>
              </div>
            ))}
          </div>
        </div>
